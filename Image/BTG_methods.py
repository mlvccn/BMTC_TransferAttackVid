...
class Attack(object):
    ...
    def _transform_img(self, img, mode='forward'):
        r'''
        Transform the img into [0, 1]
        '''
        dtype = img.dtype
...
class BTG(Attack):
    ...
    def forward(self, imgs, labels):
        ...
        # 获取未归一化的视频，用于后续扰动裁剪
        #unnorm_imgs = self._transform_img(imgs.clone().detach(), mode='back')  # [0, 1]
        
        # 初始化对抗样本
        adv_imgs = imgs.float().detach()
        
        batch_size, channels, height, width = adv_imgs.shape
        ...
        for step in range(self.steps):
            ...
            # 存储每个帧的分类损失（用于后续计算平均）
            cls_loss = self.model(adv_imgs)
            
            # 对每个帧进行处理
            for t in range(frames):
                # Do not use detach()
                x_t_adv = adv_imgs[:, :, t, :, :].clone().requires_grad_(True)
                # Forward pass and loss computation
                x_t_adv_expanded = x_t_adv.unsqueeze(2)
                if (self.model.__class__.__name__ == 'I3D_ResNetV1'):
                    x_t_adv_expanded.repeat(x_t_adv_expanded.shape[0], x_t_adv_expanded.shape[1], 16, x_t_adv_expanded.shape[3], x_t_adv_expanded.shape[4])
                output_t = self.model(x_t_adv_expanded)
                loss_t = self.loss_fn(output_t, labels)
                # Backward pass
                self.model.zero_grad()
                loss_t.backward()
                grad = x_t_adv.grad.data.clone()
                # Normalize and store grad_direction
                ...
                # 更新第 t 帧的对抗样本（暂时不更新，等总损失计算完后一起更新）
                #adv_imgs[:, :, t, :, :] = x_t_adv.detach()
            ...
            # 计算总的分类损失
            #total_cls_loss = sum(cls_losses) / frames
            
            # 总损失 = 分类损失 + 余弦相似度损失
            #total_loss = cls_loss + self.gamma * cos_sim_loss
            ...
            loss_total += self.gamma * cos_sim_loss  # 加上余弦相似度损失
            
            # 反向传播
            self.model.zero_grad()
            loss_total.backward()
            grad = adv_imgs.grad.data.clone()
            
            # 更新对抗样本
            #adv_imgs = self._transform_img(adv_imgs.detach(), mode='back')  # [0, 1]
            adv_imgs = adv_imgs + self.step_size.view(adv_imgs.shape[0], adv_imgs.shape[1], 1, 1, 1) * grad.sign()
            #delta = torch.clamp(adv_imgs - unnorm_imgs, min=-self.epsilon, max=self.epsilon)
            #adv_imgs = torch.clamp(unnorm_imgs + delta, min=0, max=1).detach()
            #adv_imgs = self._transform_img(adv_imgs, mode='forward')  # norm
            #adv_imgs = torch.clamp(imgs, imgs - self.epsilon.view(adv_imgs.shape[0], adv_imgs.shape[1], 1, 1, 1), imgs + self.epsilon.view(adv_imgs.shape[0], adv_imgs.shape[1], 1, 1, 1))
            
            delta = torch.clamp(adv_imgs - unnorm_imgs, min=-self.epsilon, max=self.epsilon)
            adv_imgs = torch.clamp(unnorm_imgs + delta, min=0, max=1).detach()
            
            # 打印损失信息
            #print(f'Step [{step+1}/{self.steps}], Classification Loss: {cls_loss:.4f}, Cosine Similarity Loss: {cos_sim_loss.item():.4f}')
