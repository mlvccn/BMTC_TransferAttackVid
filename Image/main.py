...
def get_parser():
    ...
    parser.add_argument('--eps', default=4 / 255, type=float, help='the stepsize to update the perturbation')
    parser.add_argument('--alpha', default=0.4 / 255, type=float, help='the stepsize to update the perturbation')
    ...
    parser.add_argument('--mixer_data_dir', default=None)
...
def main():
    args = get_parser()
    if args.mixer_data_dir:
        output_dir = os.path.join('',args.attack,args.model)
    else:
        output_dir = os.path.join(args.output_dir, args.attack, args.model)
    os.environ["CUDA_VISIBLE_DEVICES"] = args.GPU_ID
    if not os.path.exists(output_dir):
        os.makedirs(output_dir)

    dataset = AdvDataset(input_dir=args.input_dir, output_dir=output_dir, targeted=args.targeted, eval=args.eval, mixer_data_dir=args.mixer_data_dir)
    ...
    if not args.eval:
        ...
        attacker = transferattack.load_attack_class(args.attack)(model_name=args.model, targeted=args.targeted, epsilon = args.eps, alpha = args.alpha)
        ...
        asr = dict()
        res = '|'
        for model_name, model in load_pretrained_model(cnn_model_paper, cnn_model_pkg):
            ...
            res += ' {:.2f} |'.format(asr[model_name])

        print(asr)
        print(res)
        with open('results_eval.txt', 'a') as f:
            f.write(output_dir + res + '\n')
